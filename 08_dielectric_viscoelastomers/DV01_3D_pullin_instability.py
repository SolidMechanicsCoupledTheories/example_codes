...
t        = 0.0           # start time (s)
rampRate = 1e1          # ramp rate for voltage (1/s)
...
#
plt.plot(normVolts, stretch, linewidth=1.0, marker='.' )
...
plt.tight_layout()
plt.savefig("results/electro-viscoelastic_pullin_instability.png", dpi=600)
...
def animate(i):
    ...
    plt.plot(normVolts[0:i], stretch[0:i], linewidth=1.5)
    if i>=ii-1:
        plt.scatter(normVolts[ii-1], stretch[ii-1], c='k', marker='x', s=100)
    plt.grid(linestyle="--", linewidth=0.5, color='b')
    ax = plt.gca()
...
f = r"media/electro-viscoelastic_pullin_instability_curve.mp4"
