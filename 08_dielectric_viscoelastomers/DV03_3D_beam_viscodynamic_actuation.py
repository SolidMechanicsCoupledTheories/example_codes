...
Problem: Electro-viscoelastic dynamic actuation of a VHB beam.
...
t        = 0.0        # start time (s)
Ttot     = 5.0        # total simulation time (s) 
phitot   = 5.0        # final phi (number in kV)
tau_phi  = Ttot/20    # decay time constant for phi signal
#
numSteps = 250
...
phiRamp = Expression(("phi_tot*(1-exp(-t/tau_phi))"),
                    t = 0.0, tau_phi = tau_phi, phi_tot = phitot,Ttot=Ttot, degree=1)
...
# Output file setup
file_results = XDMFFile("results/electro_viscoelastic_3D_beam_actuate.xdmf")
...
bcs_1 = DirichletBC(ME.sub(0).sub(1), 0, facets, 2)  # u2 fix - left
bcs_2 = DirichletBC(ME.sub(0).sub(2), 0, facets, 2)  # u3 fix - left
#
# bcs_2 = DirichletBC(ME.sub(0),  Constant((0, 0, 0)), Ground, method='pointwise') # fix the origin
#
bcs_3 = DirichletBC(ME.sub(2), 0, facets, 6)  # phi ground - Bottom
bcs_4 = DirichletBC(ME.sub(2), 0, facets, 7)  # phi ground - Center
bcs_5 = DirichletBC(ME.sub(2), phiRamp, facets, 5)  # phi ramp - top
...
while (round(t + dt, 9) <= Ttot):
    ...
    timeHist0[ii] = w.sub(0).sub(1)(length, width/2, width/2) # time history of displacement
    timeHist1[ii] = w.sub(2)(length/2, width, width/2)  # time history of voltage (phi)
    timeHist2[ii] = t # store current time
...
plt.tight_layout()
plt.savefig("results/electro-viscoelastic_beam_actuation.png", dpi=600)
