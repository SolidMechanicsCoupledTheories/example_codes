...
Relative permittivity from Pelrine et al., Science (2000).
...
# Output file setup
file_results = XDMFFile("results/electro_viscoelastic_3D_helix_actuate_v3.xdmf")
...
plt.tight_layout()
plt.savefig("results/electro-viscoelastic_helix_actuate_v3.png", dpi=600)
...
def animate(i):
    ...
    ax1.plot(timeHist2[0:ind], timeHist1[0:ind], c=colors[3], linewidth=1.5)
    ax1.grid(linestyle="--", linewidth=0.5, color='b')
    ax1.set_ylabel('Electric potential (kV)')
    # ax1.set_xlim([0,5])
    from matplotlib.ticker import AutoMinorLocator,FormatStrFormatter
    ...
    ax2.plot(timeHist2[0:ind], timeHist0[0:ind], c=colors[0], linewidth=1.5)
    ax2.grid(linestyle="--", linewidth=0.5, color='b')
    ax2.set_xlabel('Time (s)')
    ax2.set_ylabel('Displacement (mm)')
    from matplotlib.ticker import AutoMinorLocator,FormatStrFormatter
