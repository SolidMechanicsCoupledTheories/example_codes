...
Problem: Electro-viscoelastic actuation of a helical VHB device. 
...
freq   = 1 # actuation frequency, Hz
nCycle = 5 # number of actuation cycles
period = 1/freq
Ttot   = nCycle*period        # total simulation time (s) , nCycle periods
#
numSteps = 100*nCycle
...
phiRamp = Expression(("phiTot*(1-cos(2*pi*f*t))/2"),
                    t = 0.0, f=freq, phiTot = phiTot, degree=1)
...
# Output file setup
file_results = XDMFFile("results/electro_viscoelastic_3D_helix_actuate_sine.xdmf")
...
#
# Optionally, the following two lines fix the base but allow the base to thin.
# bcs_0 = DirichletBC(ME.sub(0).sub(1), 0, mf, 539)  # u1 fix - base face
# bcs_1 = DirichletBC(ME.sub(0),  Constant((0, 0, 0)), Ground, method='pointwise') 
#
...
while (round(t + dt, 9) <= Ttot):
    ...
    if converged: 
        ...
        if ii%1 == 0:      
            ...
            print("Simulation Time: {} s | dt: {} s".format(round(t,4), round(dt, 5)))
            print()   
        
        # Iteration-based adaptive time-stepping
        #
        # If the newton solver takes 3 or fewer iterations,
        # and also the time step is less than 1/50th of the period time,
        # increase the time step by a factor of 1.5:
        if (iter<=3 and dt<(Ttot/50)):
            dt = 1.5*dt
            dk.assign(dt)
...
# timeHist2 = np.linspace(0, 5, 500)
# timeHist0 = np.zeros(500)
# timeHist1 = 2*(1-np.cos(2*np.pi*timeHist2))/2
...
plt.tight_layout()
plt.savefig("results/electro-viscoelastic_helix_actuate_sine.png", dpi=600)


"""
# Save time histories to an external file:
timeHist = np.stack([timeHist0, timeHist1, timeHist2])
np.savetxt("media/helix_actuate_0p05Hz.csv", timeHist, delimiter=",")
...
def animate(i):
    ...
    ax2.xaxis.set_minor_locator(AutoMinorLocator())
    ax2.yaxis.set_minor_locator(AutoMinorLocator())
    ax2.set_ylim(-5.1, 0.1)
    #plt.show()

set_fps = 50
ani = FuncAnimation(fig, animate, interval=1000/set_fps, save_count=ind)

f = r"media/electro-viscoelastic_beam_actuation_curve.mp4"
