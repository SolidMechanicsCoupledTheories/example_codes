...
Problem: Electro-viscoelastic actuation of a six-arm VHB gripper.
...
with XDMFFile("meshes/gripper_six.xdmf") as infile:
    infile.read(mesh)

# Read the 2D subdomain data stored in the *.xdmf file
mvc2d = MeshValueCollection("size_t", mesh, 2)
with XDMFFile("meshes/facet_gripper_six.xdmf") as infile:
...
class zFix(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[2], 0.5) and (x[0] <= 10) and (x[1] <= 10) and on_boundary 
...
phitot = 8.5 # kV
t      = 0.0                  # start time (s)
freq   = 4                    # frequency in Hz, cycles/s
...
'''''''''''''''''''''
# Output file setup
file_results = XDMFFile("results/3D_electro_viscoelastic_six_arm_gripper.xdmf")
...
#
bcs_0 = DirichletBC(ME.sub(0),  Constant((0, 0, 0)), Ground, method='pointwise') # fix the origin
#bcs_0 = DirichletBC(ME.sub(0),  Constant((0, 0, 0)), facets, 1) # fix the zFix plane
#
bcs_1 = DirichletBC(ME.sub(2), 0, mf, 73)  # phi ground - midplane
# bcs_2 = DirichletBC(ME.sub(2), phiAmp, mf, 74)  # phi ramp - Zbot
bcs_2 = DirichletBC(ME.sub(2), phiAmp, mf, 72)  # phi ramp - Ztop
#
bcs_3 = DirichletBC(ME.sub(0).sub(0), 0, mf, 75) # x-symmetry condition
bcs_4 = DirichletBC(ME.sub(0).sub(1), 0, mf, 76) # x-symmetry condition
...
while (round(t + dt, 9) <= Ttot):
    ...
    timeHist1[ii] = w.sub(2)(0, 0, 0.5)       # time history of voltage (phi)
    timeHist2[ii] = w.sub(0).sub(2)(0, 50, 0) # time history of displacement
...
ax1.plot(timeHist0[0:ind], timeHist1[0:ind], c=colors[3], linewidth=1.0, marker='.')
...
ax2.plot(timeHist0[0:ind], timeHist2[0:ind], c=colors[0], linewidth=1.0, marker='.')
...
plt.tight_layout()
plt.savefig("results/electro-viscoelastic_sixArm_gripper.png", dpi=600)
