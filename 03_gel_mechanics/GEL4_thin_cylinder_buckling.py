...
Import mesh
"""
# Thin cylinder parameters in mm 
H_0 = 0.6
D_0 = 4.636/2
t_0 = 0.206
...
with XDMFFile("meshes/3D_thin_cyl_mesh.xdmf") as infile:
    infile.read(mesh)
...
def face1(x, on_boundary):
    return (near(x[0], 0)) and on_boundary
def face2(x, on_boundary):
    return (near(x[1], 0)) and on_boundary
def face3(x, on_boundary):
    return near(x[2], 0) and on_boundary
def face4(x, on_boundary):
    return near(x[2], H_0) and on_boundary
...
Ttot = 900     # total simulation time 
ttd  = 60.0    # Decay time constant
...
#
bcs_1 = DirichletBC(ME.sub(0),encastre, face3)     # u1, u2, u3 fixed  on face3
#
bcs_2 = DirichletBC(ME.sub(0).sub(0), 0, face1)    # u_1 fixed on face1 
bcs_3 = DirichletBC(ME.sub(0).sub(1), 0, face2)    # u_2 fixed on face2 
#
bcs_4 = DirichletBC(ME.sub(2), muAmp, face4)       # chem. pot. on face4
# 
...
'''''''''''''''''''''
# Output file setup
file_results = XDMFFile("results/gel_3d_thin_cylinder_swell.xdmf")
