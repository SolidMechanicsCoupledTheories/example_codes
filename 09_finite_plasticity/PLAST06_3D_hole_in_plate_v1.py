...
with XDMFFile("meshes/3D_hip_v3.xdmf") as infile:
    infile.read(mesh)

# Read the 2D subdomain data stored in the *.xdmf file
mvc2d = MeshValueCollection("size_t", mesh, 2)

with XDMFFile("meshes/facet_3D_hip_v3.xdmf") as infile:
...
#
file_results = XDMFFile("results/3D_hole_in_plate.xdmf")
...
bcs_1 = DirichletBC(ME.sub(0), 0, facets, 33)  # u1 fix - xbot
bcs_2 = DirichletBC(ME.sub(1), 0, facets, 34)  # u2 fix - ybot
#

bcs_3 = DirichletBC(ME.sub(0), dispRamp, facets, 35)  # dispRamp - xtop
bcs_4 = DirichletBC(ME.sub(1), 0, facets, 35)         # u2 fix - xtop
bcs_5 = DirichletBC(ME.sub(2), 0, facets, 35)         # u3 fix - xtop
...
plt.tight_layout()
plt.savefig("results/3D_hole_in_plate.png", dpi=600)
