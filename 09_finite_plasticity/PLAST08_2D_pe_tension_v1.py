...
DEFINE GEOMETRY
'''''''''''''''''''''
yDim = Constant(17)  # Length mm
xDim = Constant(5)   # Width mm
#
# Initialize an empty mesh object
mesh = Mesh()

# Read the .xdmf  file data into mesh object
with XDMFFile("meshes/2d_tension.xdmf") as infile:
    infile.read(mesh)
...
with XDMFFile("meshes/facet_2d_tension.xdmf") as infile:
...
'''''''''''''''''''''
# Output file setup
file_results = XDMFFile("results/2D_pe_tension.xdmf")
...
bc1 = DirichletBC(ME.sub(0), Constant(0.0), facets, 7)  #  u1 fix  left
bc2 = DirichletBC(ME.sub(1), Constant(0.0), facets, 8)  #  u2 fixbot

bc3 = DirichletBC(ME.sub(0), Constant(0.0), facets, 9)  #  u1 fix top
bc4 = DirichletBC(ME.sub(1), dispRamp,      facets, 9)  #  u2 move top
...
while  round(t, 2) < round(Ttot,2):
    ...
    if converged: 
        ...
        Force    = dot(traction,n)*ds(9)    
        # Store the results for displacement and Force in array Forc
        timeHist1[ii]   = uMax*(t/Ttot)      
        timeHist2[ii]   = 2*assemble(Force)  # Convert from UFL operator to a number    
...
plt.tight_layout()
plt.savefig("results/2d_pe_tension.png", dpi=600)
