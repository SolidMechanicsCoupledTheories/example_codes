...
DEFINE GEOMETRY
'''''''''''''''''''''
# Dimensions of thespecimen
# #
# L = Constant(17)  # Height mm
# R = Constant(5)   #  width mm
...
with XDMFFile("meshes/3D_axi_tension.xdmf") as infile:
    infile.read(mesh)
...
with XDMFFile("meshes/facet_3D_axi_tension.xdmf") as infile:
    infile.read(mvc_2d, "name_to_read")
    
# Store boundary facets data
facets = cpp.mesh.MeshFunctionSizet(mesh, mvc_2d)

# # Gmsh labels
# //+
# Physical Surface("ybot", 48)  
# //+
# Physical Surface("ytop", 49)  
# //+
# Physical Surface("zbot", 50)  
# //+
# Physical Surface("xbot", 51)  
...
'''''''''''''''''''''
# Output file setup
file_results = XDMFFile("results/3D_axi_tension.xdmf")
...
bc1 = DirichletBC(ME.sub(1), Constant(0.0), facets, 48)  #  u2 fix ybot

bc2 = DirichletBC(ME.sub(0), Constant(0.0), facets, 51)  #  u1 fix xbot

bc3 = DirichletBC(ME.sub(2), Constant(0.0), facets, 50)  #  u3 fix zbot

bc4 = DirichletBC(ME.sub(0), Constant(0.0), facets, 49)  #  u1 fix top

bc5 = DirichletBC(ME.sub(1), dispRamp,      facets, 49)  #  u2 move top

bc6 = DirichletBC(ME.sub(2), Constant(0.0), facets, 49)  #  u3 fix top
...
plt.tight_layout()
plt.savefig("results/3D_axi_tension.png", dpi=600)
