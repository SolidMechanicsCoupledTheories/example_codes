...
'''''''''''''''''''''
# Output file setup
file_results = XDMFFile("results/3D_cylinder_tension.xdmf")
...
bc4 = DirichletBC(ME.sub(0), dispRamp,      facets, 3)  #  u1  move top
bc5 = DirichletBC(ME.sub(1), Constant(0.0), facets, 3)  #  u2 fix top
bc6 = DirichletBC(ME.sub(2), Constant(0.0), facets, 3)  #  u3 fix top
...
# Check if solver converges
    if converged: 
        ...
        timeHist1[ii] = uMax*2*abs(t/Ttot - np.floor(t/Ttot + 0.5))  
...
plt.tight_layout()
plt.savefig("results/3D_cylinder_tension.png", dpi=600)
