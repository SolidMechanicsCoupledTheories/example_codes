...
Problem: electro-elastic actuation of a four-arm gripper.
...
with XDMFFile("meshes/gripper.xdmf") as infile:
    infile.read(mesh)

# Read the 2D subdomain data stored in the *.xdmf file
mvc2d = MeshValueCollection("size_t", mesh, 2)
with XDMFFile("meshes/facet_gripper.xdmf") as infile:
...
while (round(t + dt, 9) <= Ttot):
    ...
    if converged: 
        ...
        timeHist2[ii] = w.sub(0).sub(2)(50, 0, 0) # time history of displacement
...
# ind = np.argmax(timeHist2)

# fig, (ax1, ax2) = plt.subplots(2,1, sharex='col')

# ax1.plot(timeHist2[0:ind+1], timeHist1[0:ind+1], c=colors[3], linewidth=2.0, marker='.')
...
# ax2.plot(timeHist2[0:ind+1], timeHist0[0:ind+1], c=colors[0], linewidth=2.0, marker='.')
# ax2.grid(linestyle="--", linewidth=0.5, color='b')
# ax2.set_xlabel('Time (s)')
# ax2.set_ylabel('Tip Disp. (mm)')
# from matplotlib.ticker import AutoMinorLocator,FormatStrFormatter
...
# plt.tight_layout()
# plt.savefig("results/Gripper_actuate.png", dpi=600)
