"""
2D axisymmetric code for finite electro-elasticity
...
#
phiTot = 1.6* float(length*np.sqrt(float(Geq_0)/float(vareps)))  # final normalized value of phi
...
Res_1 =  dot((p/Kbulk + ln(J)/J) , p_test)*x[0]*dx
...
'''''''''''''''''''''
# Output file setup
file_results = XDMFFile("results/2D_axi_pullin.xdmf")
...
plt.tight_layout()
plt.savefig("results/2D_axi_pullin_instability.png", dpi=600)
