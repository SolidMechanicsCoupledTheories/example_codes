...
Problem: Buckling of a 3-D VHB beam including dynamic and viscoelastic effects.
...
length = 20. # mm
width  = 1.   # mm
mesh   = BoxMesh(Point(0.,0.,0.),Point(width, width, length),2,2,20)
...
disp_tot = -0.5    # final tip displacement (mm)
Tcutoff = Ttot/2   # time to stop ramp and hold displacement (s)

# Boundary condition to ramp up displacement
dispAmp = Expression(("t<tC ? disp_tot*(t/tC) : disp_tot"),
                    t = 0.0, tC = Tcutoff, disp_tot = disp_tot, Ttot=Ttot, degree=1)
...
# Output file setup
file_results = XDMFFile("results/viscoelastic_beam_buckle.xdmf")
...
bcs_3 = DirichletBC(ME.sub(0).sub(2), 0, facets, 2)  # u3 fix - Bottom
#
bcs_4 = DirichletBC(ME.sub(0).sub(2), dispAmp, facets, 4)  # u1 disp - Top
bcs_5 = DirichletBC(ME.sub(0).sub(0), 0., facets, 4)  # u2 fix - Top
bcs_6 = DirichletBC(ME.sub(0).sub(1), 0., facets, 4)  # u3 fix - Top
...
while (round(t + dt, 9) <= Ttot):
    ...
    # Calculate the reaction force at the moved boundary:    
    n        = FacetNormal(ds)
    Tmat_new = T_mat_calc(u, p, A1, A2, A3)
    traction = dot(Tmat,n)
    Force    = dot(traction,n)*ds(4)
    
    # Store force and displacement at this time
    timeHist0[ii] = w.sub(0).sub(2)(0.5,0.5, 20.0)
    timeHist1[ii] = assemble(Force)
...
plt.tight_layout()
plt.savefig("results/viscoelastic_beam_buckle.png", dpi=600)
...
def animate(i):
    ...
    ax2.plot(timeHist2[0:i], -timeHist1[0:i], c=colors[3], linewidth=1.5)
    ax2.grid(linestyle="--", linewidth=0.5, color='b')
    ax2.set_ylabel('Force (N)')
    ax2.set_xlabel('Time (s)')
    from matplotlib.ticker import AutoMinorLocator,FormatStrFormatter
    ...
    ax2.set_xlim(-0.025, 0.275)
    
    ax1.plot(timeHist2[0:i], -timeHist0[0:i], c=colors[0], linewidth=1.5)
    ax1.grid(linestyle="--", linewidth=0.5, color='b')
    ax1.set_ylabel('Displacement (mm)')
    from matplotlib.ticker import AutoMinorLocator,FormatStrFormatter
    ax1.xaxis.set_minor_locator(AutoMinorLocator())
...
f = r"media/viscoelastic_beam_buckle_curve.mp4"
