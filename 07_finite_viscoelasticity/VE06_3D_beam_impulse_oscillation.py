...
Problem: Dissipative oscillations of a 3-D VHB beam with a square cross-section
         in response to an impulse loading at the tip.
...
# Traction pulse paramters
trac_tot = 0.1                   # final shear traction on right face (kPa)
Tcutoff  = Ttot*(4.0/numSteps)   # cutoff time for the traction "pulse"

# Boundary condition expression for the traction "pulse"
tracAmp = Expression(("t<tC ? trac_tot*2*abs(t/tC - floor(t/tC + 0.5)) : 0"),\
                    t = 0.0, tC = Tcutoff, trac_tot = trac_tot, Ttot=Ttot, degree=1)
...
# Output file setup
file_results = XDMFFile("results/viscoelastic_beam_impulse.xdmf")
...
# BC set, left end built-in
bcs = DirichletBC(ME.sub(0), (0.0, 0.0, 0.0), facets, 2) 
...
plt.tight_layout()
plt.savefig("results/viscoelastic_beam_oscillation.png", dpi=600)
...
f = r"media/viscoelastic_beam_oscillation_curve.mp4"
