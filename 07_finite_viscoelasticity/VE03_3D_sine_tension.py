...
Problem:  Sinusoidal uniaxial displacement of cube
...
lam_max = 1.5                # stretch amplitude
uMax =  (lam_max-1)*L        # dispalcement amplitude. Remember L is the size of the unit box
# Time history
t      = 0.0                  # start time (s)
...
# Output file setup
file_results = XDMFFile("results/viscoelastic_periodic_tension.xdmf")
...
# Boundary condition definitions, homogeneous tension
...
#
#
bcs_4 = DirichletBC(ME.sub(0).sub(1), dispAmp, facets, 4)  # u2 move   - yTop
...
fig = plt.figure() 
ax=fig.gca()  
plt.plot(timeHist0[0:ind]+1, timeHist1[0:ind], c='b', linewidth=1.0, marker='.')
#-------------------------------------------------------------
#ax.set.xlim(-0.05,0.05)
#ax.set.ylim(-0.03,0.03)
#plt.axis('tight')
plt.grid(linestyle="--", linewidth=0.5, color='b')
ax.set_xlabel("Stretch",size=14)
ax.set_ylabel("Engineering stress (kPa)",size=14)
#ax.set_ylim([0, 60])
...
plt.tight_layout()
plt.savefig("results/viscoelastic_periodic_tension.png", dpi=600)
...
def animate(i):
    ...
    plt.plot(timeHist0[0:i]+1, timeHist1[0:i], c='b', linewidth=1.0)
    #-------------------------------------------------------------
    plt.xlim(0.45,1.55)
    plt.ylim(-300,150)
    plt.grid(linestyle="--", linewidth=0.5, color='b')
    plt.xlabel("Stretch",size=14)
    plt.ylabel("Engineering stress (kPa)",size=14)

set_fps = 24
ani = FuncAnimation(fig, animate, interval=1000/set_fps, save_count=ind+1)

f = r"media/periodic_tension_curve.mp4"
