...
Problem:  Sinusoidal shear of a unit cube of VHB.
...
L= 1.0 
# A basic box mesh 
mesh = BoxMesh(Point(0.,0.,0.),Point(L,L,L),8,8,3)
...
#
dispAmp = Expression(("uMax*sin(omega*t)"), omega = omega,\
                      uMax = uMax, t = 0.0, degree=1)
# Time step
...
# Output file setup
file_results = XDMFFile("results/viscoelastic_periodic_shear.xdmf")
...
def writeResults(t):
    ...
    TR12_Vis = project(Tmat[0,1],W)
    TR12_Vis.rename("TR12"," ")
...
bcs_1 = DirichletBC(ME.sub(0).sub(0), 0, facets, 3)  # u1 fixed - yBot
bcs_2 = DirichletBC(ME.sub(0).sub(1), 0, facets, 3)  # u2 fixed - yBot
bcs_3 = DirichletBC(ME.sub(0).sub(2), 0, facets, 3)  # u3 fixed - yBot
#
#
bcs_4 = DirichletBC(ME.sub(0).sub(0), dispAmp, facets, 4)  # u1 move  -  yTop
bcs_5 = DirichletBC(ME.sub(0).sub(1), 0, facets, 4)  # u2 fix    - yTop
bcs_6 = DirichletBC(ME.sub(0).sub(2), 0, facets, 4)  # u3 fixe   - yTop
...
while (round(t + dt, 9) <= Ttot):
    ...
    timeHist0[ii] =  uMax*sin(omega*t)  #  Applied axial displacement
    timeHist1[ii] =  assemble(Force)       #  Convert from UFL operator to a number in milli Newtons
...
fig = plt.figure() 
ax=fig.gca()  
plt.plot(timeHist0[0:ind]/L, timeHist1[0:ind], c='b', linewidth=1.0, marker='.')
#-------------------------------------------------------------
#ax.set.xlim(-0.05,0.05)
#ax.set.ylim(-0.03,0.03)
#plt.axis('tight')
plt.grid(linestyle="--", linewidth=0.5, color='b')
ax.set_xlabel(r'Shear strain',size=14)
ax.set_ylabel(r'Shear stress, kPa ',size=14)
#ax.set_ylim([0, 60])
#ax.set_title("Force-dispalcement curve", size=14, weight='normal')
...
plt.tight_layout()
plt.savefig("results/viscoelastic_periodic_shear.png", dpi=600)
...
def animate(i):
    ...
    plt.plot(timeHist0[0:i]/L, timeHist1[0:i], c='b', linewidth=1.0)
    #-------------------------------------------------------------
    plt.xlim(-1.05,1.05)
    plt.ylim(-75, 75)
    plt.grid(linestyle="--", linewidth=0.5, color='b')
    plt.xlabel(r'Shear strain',size=14)
    plt.ylabel(r'Shear stress, kPa ',size=14)

set_fps = 24
ani = FuncAnimation(fig, animate, interval=1000/set_fps, save_count=ind+1)

f = r"media/periodic_shear_curve.mp4"
