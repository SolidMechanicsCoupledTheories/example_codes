...
Problem:  Creep of a unit cube of VHB.
...
tracMax =  100.0        # maximum traction applied, kPa
# Time history
t      = 0.0                  # start time (s)
tRamp  = 1                    # ramp time for displacement
Ttot   = 10                   # total time
#
tracAmp = Expression(("min(tracMax*t/tRamp, tracMax)"),\
                      tracMax = tracMax, t = 0.0, tRamp = tRamp, degree=1)
# Time step
numSteps = 200 
...
#
# Applied normal traction on top face
...
# Output file setup
file_results = XDMFFile("results/viscoelastic_creep.xdmf")
...
while (round(t + dt, 9) <= Ttot):
    ...
    timeHist0[ii] =  w.sub(0).sub(1)(L, L, L) #  axial displacement
    timeHist1[ii] =  assemble(Force)       #  Convert from UFL operator to a number in  Newtons
...
ax1.plot(timeHist2[0:ind], timeHist1[0:ind]/(L*L), c=colors[3], linewidth=1.0, marker='.')
ax1.grid(linestyle="--", linewidth=0.5, color='b')
ax1.set_ylabel('Engineering Stress (kPa)')
...
plt.tight_layout()
plt.savefig("results/viscoelastic_creep.png", dpi=600)
...
def animate(i):
    ...
    ax1.plot(timeHist2[0:i], timeHist1[0:i]/(L*L), c=colors[3], linewidth=1.5)
    ax1.grid(linestyle="--", linewidth=0.5, color='b')
    ax1.set_ylabel('Engineering Stress (kPa)')
    from matplotlib.ticker import AutoMinorLocator,FormatStrFormatter
    ax1.xaxis.set_minor_locator(AutoMinorLocator())
