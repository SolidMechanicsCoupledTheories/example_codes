...
Problem:  Stress relaxation of a unit cube of VHB.
...
# Time history
t      = 0.0                  # start time (s)
tRamp  = 1                    # ramp time for displacement
Ttot   = 10                   # total time
#
dispAmp = Expression(("min(uMax*t/tRamp, uMax)"),\
                      uMax = uMax, t = 0.0, tRamp = tRamp, degree=1)
# Time step
numSteps = 200 
...
# Output file setup
file_results = XDMFFile("results/viscoelastic_stress_relaxation.xdmf")
...
while (round(t + dt, 9) <= Ttot):
    ...
    timeHist0[ii] =  min(uMax*t/tRamp, uMax)  #  Applied axial displacement
...
fig = plt.figure() 
ax=fig.gca()  
plt.plot(timeHist2[0:ind], timeHist1[0:ind], c='b', linewidth=1.0, marker='.')
#-------------------------------------------------------------
...
plt.tight_layout()
plt.savefig("results/viscoelastic_stress_relaxation.png", dpi=600)
...
def animate(i):
    ...
    plt.plot(timeHist2[0:i], timeHist1[0:i], c='b', linewidth=1.0)
    #-------------------------------------------------------------
    plt.xlim(0,10)
    plt.ylim(0,100)
    plt.grid(linestyle="--", linewidth=0.5, color='b')
...
f = r"media/stress_relaxation_curve.mp4"
