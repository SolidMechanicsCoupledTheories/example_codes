...
Inflation of a cylinderical shell
...
# Cylinder dimensions
R0 = Constant(10) # mm inner radius
T0 = Constant(1)  # mm wall thickness
L0 = Constant(5)  # mm cylinder length

# Read the *.xdmf file data into mesh object
with XDMFFile("meshes/cylinder_inflate.xdmf") as infile:
    infile.read(mesh)

# Read the 2D subdomain data stored in the *.xdmf file
mvc2d = MeshValueCollection("size_t", mesh, 2)
with XDMFFile("meshes/facet_cylinder_inflate.xdmf") as infile:
...
#
file_results = XDMFFile("results/3D_cylinder_inflate.xdmf")
...
bcs_1 = DirichletBC(ME.sub(0).sub(0), 0, facets, 30)  # u1 fix - left_top
bcs_2 = DirichletBC(ME.sub(0).sub(1), 0, facets, 29)  # u2 fix - right_bot
bcs_3 = DirichletBC(ME.sub(0).sub(2), 0, facets, 32)  # u3 fix - ztop
bcs_4 = DirichletBC(ME.sub(0).sub(2), 0, facets, 33)  # u3 fix - zbot
...
while (round(t + dt, 9) <= Ttot):
    ...
    timeHist2[ii] = w.sub(0).sub(0)(R0, 0, 0)         # time history of displacement of 
                                                      # selected point
