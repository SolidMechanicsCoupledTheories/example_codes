...
# A 3-D cube
length = 1. # mm
mesh = BoxMesh(Point(0.,0.,0.),Point(length,length,length*20),4,4,20)

# Extract initial mesh coords
x = SpatialCoordinate(mesh)

# Identify the plane boundaries of the  box mesh
#
...
class zTop(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[2],20*length) and on_boundary
...
# Expression for dispalcement ramp
...
'''''''''''''''''''''
# Output file setup
file_results = XDMFFile("results/3D_buckling_fixed.xdmf")
...
BOUNDARY CONDITIONS 
"""
# Recall the markers for the facets
#
# xBot().mark(facets, 1)
# xTop().mark(facets, 2)
# yBot().mark(facets, 3)
...
#
bcs_1 = DirichletBC(ME.sub(0), Constant((0,0,0)), facets, 5)  # disp. fix - zBot
#
bcs_2 = DirichletBC(ME.sub(0).sub(2), dispRamp, facets, 6)  # u3 ramp - zTop
bcs_3 = DirichletBC(ME.sub(0).sub(0), 0, facets, 6)  # u1 fix - zTop
bcs_4 = DirichletBC(ME.sub(0).sub(1), 0, facets, 6)  # u2 fix - zTop
...
while (round(t + dt, 9) <= Ttot):
    ...
    # Store  displacement of a point on zTop
    timeHist0[ii] = w.sub(0).sub(2)(length, length, 20*length) 
    # Store  time history of Piola stress
    P33 = project(Tmat[2,2],W)
    timeHist1[ii] = assemble(P33*ds(6))/(length*length)
...
ax=fig.gca()  
#--------------------------------------------------------------
plt.plot(timeHist0, timeHist1, linewidth=2.0,\
