...
A block under  pressure on a  part of its face
...
# A basic box mesh 
mesh = BoxMesh(Point(0.,0.,0.),Point(50.,50.,50.),10,10,6)

x = mesh.coordinates()

#Pick up on the boundary entities of the created mesh
class xFace(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[0],0) and on_boundary
class yFace(SubDomain):
    def inside(self, x, on_boundary):
...
class Top(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[2],50.) and on_boundary
class loadFace(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[2],50.) and (x[0] <= 50./2.0) and (x[1] <= 50./2.0) and on_boundary
...
# Expression to ramp up traction
tracRamp = Expression(("trac_tot*(t/Ttot)"),
                    trac_tot = trac_tot,t = 0.0, Ttot=Ttot, degree=1)
...
#--------------------------
# Cauchy stress for Arruda-Boyce material
def T_calc(u,p):
    Id = Identity(3) 
    F   = F_calc(u)
    J = det(F)
...
'''''''''''''''''''''
# Output file setup
file_results = XDMFFile("results/3D_cube_footing.xdmf")
...
bcs_1 = DirichletBC(ME.sub(0).sub(0), 0, facets, 2)  # u1 fix - xFace
bcs_2 = DirichletBC(ME.sub(0).sub(1), 0, facets, 3)  # u2 fix - yFace
bcs_3 = DirichletBC(ME.sub(0).sub(0), 0, facets, 6)  # u1 fix - loadFace
bcs_4 = DirichletBC(ME.sub(0).sub(1), 0, facets, 6)  # u2 fix - loadFace
bcs_5 = DirichletBC(ME.sub(0).sub(2), 0, facets, 4)  # u3 fix - Bottom
...
while (round(t + dt, 9) <= Ttot):
    ...
    # Store  displacement history at a particular point
    timeHist0[ii] = w.sub(0).sub(2)(0.,0.,50.) 
...
ax=fig.gca()  
plt.plot(times, timeHist0, c='b',label='Simulation', linewidth=2.0)
#-------------------------------------------------------------
