...
# Triangle mesh
mesh = RectangleMesh(Point(0., 0.), Point(L0, H0), 67, 6, "crossed")
...
chi_L_0   = Constant(0.1)                   # Flory-Huggins mixing parameter low value
chi_H_0   = Constant(0.7)                   # Flory-Huggins mixing parameter high value
# Non-thermally-responsive material
chi_L_1   = Constant(0.1)                   # Flory-Huggins mixing parameter low value
chi_H_1   = Constant(0.1)                   # Flory-Huggins mixing parameter high value
# Construct the spatially varying chi values
chi_L   = mat(materials, chi_L_0, chi_L_1, degree=0) 
chi_H   = mat(materials, chi_H_0, chi_H_1, degree=0) 
#--------------------------------------------------------------------------
#
...
step1_time = 3600    
# 
tRamp      = 1500       # ramp time for increasing / decreasing temperature
#
step2_time = 1500       # ramp time + hold time for heating step
#
step3_time = 3600*2     # ramp time + hold time for cooling  step
#
Ttot       = step1_time + step2_time  + step3_time
dt         = 50         # Fixed  time step size
...
# Initialize chemical potential, corresponding to nearly dry polymer, phi0=0.999
...
'''''''''''''''''''''
# Output file setup
file_results = XDMFFile("results/thermogel_pe_bilayer_swell.xdmf")
...
def writeResults(t):
    ...
    # Visualize chi_H_H
    chi_H_Vis = project(chi_H, W)
    chi_H_Vis.rename("chi_H"," ")
    ...
    file_results.write(chi_H_Vis, t)
...
step = "Swell"
# Initializ conter for printing output
ii = 0
...
bcs_1 = DirichletBC(ME.sub(0).sub(0), 0, left)    # u1 fix - Left  
bcs_2 = DirichletBC(ME.sub(0),  Constant((0, 0)), Ground, method='pointwise') # Fix the bottom left corner
#
bcs_3 = DirichletBC(ME.sub(2), muRamp, right)          # chem. pot. - Right
bcs_4 = DirichletBC(ME.sub(2), muRamp, top)            # chem. pot. - Top
bcs_5 = DirichletBC(ME.sub(2), muRamp, bottom)         # chem. pot. - Bottom
#
bcs_6 = DirichletBC(ME.sub(4), theta0, right)  # Temperature - Right
bcs_7 = DirichletBC(ME.sub(4), theta0, top)    # Temperature - Top
bcs_8 = DirichletBC(ME.sub(4), theta0, bottom) # Temperature - Bottom
#  
...
bcs_5 = DirichletBC(ME.sub(2), muRamp, bottom)         # chem. pot. - Bottom
#
bcs_6 = DirichletBC(ME.sub(4), thetaRamp, right)  # Temperature - Right
bcs_7 = DirichletBC(ME.sub(4), thetaRamp, top)    # Temperature - Top
bcs_8 = DirichletBC(ME.sub(4), thetaRamp, bottom) # Temperature - Bottom
#  
bcs = [bcs_1, bcs_2, bcs_3, bcs_4, bcs_5, bcs_6, bcs_7, bcs_8]
...
bcs_5 = DirichletBC(ME.sub(2), muRamp, bottom)         # chem. pot. - Bottom
#
bcs_6 = DirichletBC(ME.sub(4), thetaRamp2, right)  # Temperature - Right
bcs_7 = DirichletBC(ME.sub(4), thetaRamp2, top)    # Temperature - Top
bcs_8 = DirichletBC(ME.sub(4), thetaRamp2, bottom) # Temperature - Bottom
#  
bcs = [bcs_1, bcs_2, bcs_3, bcs_4, bcs_5, bcs_6, bcs_7, bcs_8]
