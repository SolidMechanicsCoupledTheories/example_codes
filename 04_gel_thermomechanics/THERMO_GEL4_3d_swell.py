"""
3D code for thermo-coupled hydrogels
...
# Create mesh  
N=6 # number of elements along each side
mesh = BoxMesh(Point(0,0,0), Point(L_0,L_0,L_0), N, N, N)
...
class xTop(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[0],L_0) and on_boundary
...
class yTop(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1],L_0) and on_boundary
...
class zTop(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[2],L_0) and on_boundary
...
ttd        = 500        # time constant for increasing mu
step1_time = 3600*2    
# 
...
# Res_4: Temperature    (test fxn:theta)
...
'''''''''''''''''''''
# Output file setup
file_results = XDMFFile("results/thermogel_3d_cube_swell.xdmf")
...
bcs_4 = DirichletBC(ME.sub(2), muRamp, facets, 2)     # chem. pot. - xTop
bcs_5 = DirichletBC(ME.sub(2), muRamp, facets, 4)     # chem. pot. - yTop
bcs_6 = DirichletBC(ME.sub(2), muRamp, facets, 6)     # chem. pot. - zTop

bcs_7 = DirichletBC(ME.sub(4), theta0, facets, 2)     # chem. pot. - xTop
bcs_8 = DirichletBC(ME.sub(4), theta0, facets, 4)     # chem. pot. - yTop
bcs_9 = DirichletBC(ME.sub(4), theta0, facets, 6)     # chem. pot. - zTop
# 
bcs = [bcs_1, bcs_2, bcs_3, bcs_4, bcs_5, bcs_6, bcs_7, bcs_8, bcs_9]
...
bcs_7 = DirichletBC(ME.sub(4), thetaRamp, facets, 2)     # chem. pot. - xTop
bcs_8 = DirichletBC(ME.sub(4), thetaRamp, facets, 4)     # chem. pot. - yTop
bcs_9 = DirichletBC(ME.sub(4), thetaRamp, facets, 6)     # chem. pot. - zTop
# 
bcs = [bcs_1, bcs_2, bcs_3, bcs_4, bcs_5, bcs_6, bcs_7, bcs_8, bcs_9]
...
bcs_7 = DirichletBC(ME.sub(4), thetaRamp2, facets, 2)     # chem. pot. - xTop
bcs_8 = DirichletBC(ME.sub(4), thetaRamp2, facets, 4)     # chem. pot. - yTop
bcs_9 = DirichletBC(ME.sub(4), thetaRamp2, facets, 6)     # chem. pot. - zTop
# 
bcs = [bcs_1, bcs_2, bcs_3, bcs_4, bcs_5, bcs_6, bcs_7, bcs_8, bcs_9]
